...
export default function ReturnScreen({ route }) {
    const status = route.params.status ?? 0;
    console.log(route.params)
    return (
        <View style={style.container}>
            {status === 0 && <View style={style.container}>
                <Text style={style.title}>Return List</Text>
                <ListItems item={route.params.items} json={route.params.order} textb={'Start Return'} nopickut={false}/>
            </View>}
